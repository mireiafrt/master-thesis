...
# Train with REALdata (train 5 different models), evaluate on real data --> Need to make sure proportions are same as other experiment
...
log_path = os.path.join(output["results_dir"], f"exp_multi_task.csv")
...
# Remove df_test rows from the original dataframe
df_remaining = df.drop(test_df.index)
# Count how many rows you need for each (sex, age_group) combo
group_counts = test_df.groupby(target_cols).size()
...
for i in range(0, training["num_runs"]):
    ...
    # Sample from df_remaining, matching the group sizesof test_df, no repeats
    subset_df = (df_remaining
        .groupby(target_cols, group_keys=False)
        .apply(lambda g: g.sample(n=group_counts.get(tuple(g.name), 0), random_state=i))
        .reset_index(drop=True)
    )
    print(f"Check if test_df and subset_df are equal: {len(subset_df) == len(test_df)}")
    print(test_df.groupby(target_cols).size())
    print(subset_df.groupby(target_cols).size())
    ...
    train_df, val_df = train_test_split(subset_df, train_size=split["train_size"], stratify=subset_df["stratify_key"], random_state=i)

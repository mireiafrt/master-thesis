...
train_transforms = transforms.Compose([
    ...
    # Transform to 3 channels because MedVAE needs 3 channel inputs
    transforms.LambdaD(keys=["image"], func=lambda x: x.repeat(3, 1, 1)),
    # Rescale intensity from [0, 255] to [0.0, 1.0] — common normalization step
    transforms.ScaleIntensityRanged(keys=["image"], a_min=0.0, a_max=255.0, b_min=0.0, b_max=1.0, clip=True),
    # Resize to 256x256 so latents end up being 64x64
    ...
    transforms.RandAffined(
        ...
        spatial_size=[256, 256],                 # ensure final size stays constant
        prob=0.10,
    ),
...
print(f"Loading MedVAE autoencoder")
medvae = MVAE(model_name="medvae_4_3_2d", modality="xray").to(device)
medvae.eval()
medvae.requires_grad_(False)
print_cuda_memory("After MedVAE Loaded")
...
with torch.no_grad():
    posterior = medvae.model.encode(eda_data.to(device))  # img shape: [B, 3, H, W]
    z = posterior.sample()  # shape: [B, latent_channels, h, w]
    print("Latent shape: ", z.shape)
scale_factor = 1 / torch.std(z)
print("Scale factor:", scale_factor.item())
    ...
    for step, x in pbar:
        ...
        with autocast(enabled=True):
            with torch.no_grad():
                ...
                posterior = medvae.model.encode(images)
                e = posterior.sample() * scale_factor
        ...
        with torch.no_grad():
            ...
            for x in tqdm(val_loader, desc=f"Val Epoch {epoch}"):
                ...
                with autocast(enabled=True):
                    ...
                    latent_hat = medvae.model.decode(noise_pred / scale_factor)

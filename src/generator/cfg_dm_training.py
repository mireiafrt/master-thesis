...
unet = DiffusionModelUNet(
    ...
    num_channels=(64, 64, 64),   # Number of channels at each level of the UNet
    attention_levels=(False, False, True), # Whether to apply self-attention at each UNet level
    num_res_blocks=1,             # Number of residual blocks per level in the UNet
    num_head_channels=(0, 0, 64), # Number of channels per attention head at each level (if attention is enabled)
...
for epoch in range(n_epochs):
    ...
    for step, batch in progress_bar:
        ...
        with autocast(enabled=True):
            ...
            noise_pred = inferer(
                inputs=images, diffusion_model=unet, noise=noise, timesteps=timesteps, condition=cond
            )
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        for t in tqdm(scheduler.timesteps):
            with torch.no_grad(), autocast(enabled=True):
                ...
                model_output = unet(noise_input, timesteps=torch.tensor([t], device=device), context=cond)
                noise_pred_uncond, noise_pred_cond = model_output.chunk(2)
                noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_cond - noise_pred_uncond)

            noise, _ = scheduler.step(noise_pred, t, noise)
        # save output
        plt.imsave(f"{paths['model_output']}/sample_epoch{epoch}.png", noise[0, 0].cpu(), cmap="gray", vmin=0, vmax=1)

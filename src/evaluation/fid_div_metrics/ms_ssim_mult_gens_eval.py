...
def compute_pair_metrics(real_loader, syn_loader, ms_ssim, ssim, device):
    ms_vals, ssim_vals = [], []
    ...
    for step, (real_batch, syn_batch) in tqdm(enumerate(zip(real_loader, syn_loader)), total=len(real_loader)):
        ...
        ms_vals.append(ms_ssim(real, syn).cpu())   # (B,)
        ssim_vals.append(ssim(real, syn).cpu())   # (B,)
...
for syn_loader in syn_loaders:
    ms_vals, ssim_vals = compute_pair_metrics(real_loader, syn_loader, ms_ssim, ssim, device)
...
print(f"MS-SSIM  : {ms_mean:.6f} ± {ms_ci:.6f}  (95 % CI, n={len(syn_paths)})")
print(f"SSIM     : {ssim_mean:.6f} ± {ssim_ci:.6f}  (95 % CI, n={len(syn_paths)})")

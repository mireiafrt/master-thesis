# First, train a single model on the hold out real data (train+val)

# TRTR --> Evaluate the trained model on the real test set
# TRTS---> Evaluate the trained model on the 5 synthetic test sets

# Compute the difference in metrics between each TRTR and TSTR pairs
# Compute a 95% CI of the 5 differences
...
# ========== Load Config ==========
with open("config/evaluation/general_utility_evaluating.yaml", "r") as f:
...
train_df = df[df[columns["split_col"]] == "train"]
val_df = df[df[columns["split_col"]] == "val"]
...
def evaluate_on_test(best_model_state, test_df, image_path_col):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = DenseNet121(spatial_dims=2, in_channels=3, out_channels=2, pretrained=True).to(device)
